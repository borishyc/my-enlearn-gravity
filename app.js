...
btnEditCard.addEventListener("click", () => {
  ...
  editExamplesContainer.appendChild(addExBtn);
...
async function lookupDictionary(wordToLookup) {
  ...
  try {
    ...
    if (englishDef && englishDef !== "查無此字義") {
      const trRes = await fetch(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(englishDef)}&langpair=en|zh-TW`);
      const trData = await trRes.json();
